...
var Search = Search || (function() {
	...
	const tokenizer=function(text){
		return _.chain((text||'').toLowerCase().replace(/<[^>]*>|\d+|&[a-zA-Z0-9#]*;/g,' ').trim().split(/[\W\s]+/))
			.map(stemmer)
			.reject((w)=>_.contains(stopStems,w))
			.value();
	},

    valid = function(entry){
        return (_.isString(entry) && entry.length) || (_.isNumber(entry) && !_.isNaN(entry));
    },
    message = function(msg,who='gm'){
	...
	addDocument = function(document){
        if( valid(document.id) && valid(document.category) && valid(document.field)){
            ...
            doc.terms=_.mapObject(_.countBy(doc.tokens),(c)=>({count:c,freq:(c/doc.tokens.length)}));
            ...
            corpus.categories = _.union(corpus.categories,[doc.category]);
            corpus.fields = _.union(corpus.fields,[doc.field]);

            _.each(doc.terms,(s,t)=>{
                corpus.terms[t]=(corpus.terms[t]||{n:0,idf:0});
                corpus.terms[t].n++;
                corpus.termInverseIndex[t]=(corpus.termInverseIndex[t]||{});
                corpus.termInverseIndex[t][doc.corpusid]=doc;
            });
        } else {
	...
	updateIDF = function(){
		_.each(corpus.terms,(s,t)=>{
			corpus.terms[t].idf=Math.max(Math.log10((corpus.stats.docs - s.n + 0.5) / (s.n+0.5)), 0.01);
		});
	},

    rank = function(term,stats,doc){
        return stats.idf * (
            (
	...
	search = function(operations){
        let requiredTermMap={},
			player=operations.player,
			isGM=operations.isGM,
            terms=_.reduce(operations,(m,context,key)=>{
            switch(key){
                case 'terms':
                    _.each(context,(obj,term)=>{
                ...
                case 'categories':
                    _.each(context, (catData,category)=>{
                ...
                case 'fields':
                    _.each(context,(fieldData,field)=>{
                        _.each(fieldData.terms,(obj,term)=>{
                            m[term]=(m[term]||{categories:[],categoryfields:[],fields:[],global:false});
	...
	buildOperations = function(options){
		...
		_.each(tokenList,(tok)=>{
			let type,
                match,
                mode=_.last(modeStack).split(/:/)[0],
                matchers = (_.has(permitted,mode) ? _.pick(regex,permitted[mode]) : regex);
    ...
    checkInstall = function() {
        ...
        if( ! _.has(state,'Search') || state.Search.version !== schemaVersion) {
...
`</div>`
	...
	formatResults = function(results,context){

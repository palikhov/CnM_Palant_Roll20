		...
		if (msg.type === 'api' && msg.content.search(/^!ta\b/) !== -1 && msg.selected) {
			...
			_.each(char, function(a) {
			    if (isNpc(a.id) === "1") {
			        ...
			        // Set Roll Queries: Always = "{{always=1}} {{r2=[[@{d20}" , Toggle = "@{advantagetoggle}" , Query = "{{query=1}} ?{Advantage?|Normal Roll,{{normal=1}} {{r2=[[0d20|Advantage,{{advantage=1}} {{r2=[[@{d20}|Disadvantage,{{disadvantage=1}} {{r2=[[@{d20}}" , Never = "{{normal=1}} {{r2=[[0d20"
